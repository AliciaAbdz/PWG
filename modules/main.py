# PEP8--80-characters-80-characters-80-characters-80-characters-80-characters---
...
class Password: 
    ...
    def __validate_optional_parameters_requirements(self) : 
        """ 
        Permet de valider que si les paramètres "Mixed Case" et/ou "cuts" sont 
        actifs, le paramètre essentiel "Lettre" l'est aussi.
    ...
    def __get_initial_chars_library(self) : 
        ...
        """ 
        Crée les bibliothèques de caractères disponibles pour la génération
        du mot de passe.
    ...
    def __clean_special_chars(self) : 
        ...
        Si "Cuts" est actif, il retire également les symboles "_" et "-" qui 
    ...
    def __validate_exclude_all_chars(self):
        """ 
        Permet de valider que le paramètre "Exclude" n'exclut pas toute la 
        chaine de caractères générées par les paramètres essentiels.
        ...
        if self.all_chars == self.exclude_chars :
            raise ValueError("La section \"Exclude\" comporte tous les " 
                             "caractères sélectionnés !")
        
    def __validate_conflicts_include_exclude(self):
        """ 
        Permet de valider les conflits entre les paramètres "Include" et 
        "Exclude":
            - 1) Raise une Execption s'ils sont exactement similaires.
            - 2) Exclu automatiquement les caractères similaires entre les 
        ...
        if self.include_chars and self.exclude_chars : 
            ...
            if self.include_chars == self.exclude_chars :
                raise ValueError("Les listes d'inclusion et d'exclusion sont" 
                                 "similaires")
            ...
            # Need to find longest list to correctly compare in the next for 
            # loop.
    ...
    def __get_password_chars_library(self) : 
        """ 
        Permet créer une librarie de caractères servant pour la création du 
        mot de passe.
    ...
    def __separate_chars_category(self):
        ...
        """ 1) Permet créer différentes catégories de libraries en fonction de
        ...
        """ 2) Permet également de créer une liste contenant les informations 
        sur le nombre de parametres différents dans le mot de passe 
        "chars_type_available". """
    ...
    def __apply_cuts(self, password, separator="-") : 
        ...
        """ 
        Permet à partir d'un mot de passe déjà créé de générer un autre mot de 
        passe mais séquencé et coupé par un séparateur unique.
        Ne fonctionne qu'avec un nombre minimum de caractères (8 par défaut).
        Les séparateurs sont placés de manière redondante et calculée 
        aléatoirement selon la longueur initiale du mot de passe et un diviseur 
        arbitraire.

        Une fois cette redondance calculée en terme d'index dans la liste de 
        caractères composant le mot de passe, cette dernière est modifiée puis 
        transformée en variable de type string qui surcharge le paramètre 
        "password". Cette variable est ensuite retournée à la sortie de la 
        fonction.
        ...
        if self.length >= 8 :
            ...
            warnings.warn("Cuts ignored, only available with minimum lenght of 8" 
                          "characters", stacklevel=2)

    def generate(self) : 

        """ 
        Génère un premier tour de séquence chiffrée pour créer plus tard le mot
        de passe. 
        Un chiffre random est généré dans l'espace de nombre entre 1 et le 
        nombre de parametres selectionnés par l'utilisateur. 
        Il est ensuite ajouté à la séquence chiffrée qui permettra de composer 
        le mot de passe.
        Un second tour de sécurité est lancé si un des parametres n'a pas été 
        généré durant le premier tour.
